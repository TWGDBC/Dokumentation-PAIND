\include{Industriearbeit_settings}

%%Titles - Uncomment one section of titles

%%Used for titleGraduation
\include{titles/titleGraduation}


%%Used by all titles
\title{3D Laserscanner für mobilen Roboter}
\author{Daniel Zimmermann}
\matrikelnr{15-465-271}
\submitDate{22.12.2017}
\firstExaminer{Björn Jensen}
\secondExaminer{Markus Thalmann}

%%End Titles
\makeindex[title=Stichwortverzeichnis, options=-s indexstyle.ist, intoc]
\indexsetup{level=\chapter*,toclevel=chapter}

\usepackage{filecontents}

%\loadglsentries{glossaryandacronyms.tex}
%\setacronymstyle{long-short}
%\makeglossaries
\begin{document}

\pagenumbering{alph} %fix for same identifier warning, character is not show in title
\maketitle
\pagenumbering{Roman}
% Eigenständigkeitserklärung
\fancyhead[L]{\nouppercase{Introduktion}} %Kopfzeile links
\input{chapter/Eigenstaendigkeitserklaerung}
% Vorwort
\input{chapter/Abstract} \clearpage
%\input{chapter/Einleitung} \clearpage

% Abstract

\input{Inhaltsverzeichnis} \clearpage

\newpage
\fancyhead[L]{\nouppercase{Glossar}} %Kopfzeile links
\markright{GLOSSAR}
\chapter*{Glossar}\addcontentsline{toc}{chapter}{Glossar}
\input{glossary_and_acronyms.tex}
\fancyhead[L]{\nouppercase{Abbildungsverzeichnis}} %Kopfzeile links
\listoffigures \clearpage
\fancyhead[L]{\nouppercase{Tabellenverzeichnis}} %Kopfzeile links
\listoftables \clearpage
%\lstlistoflistings \clearpage
\fancyhead[L]{\nouppercase{Literaturverzeichnis}} %Kopfzeile links
%\printbibliography[heading=bibintoc, keyword={book}, title={Literaturverzeichnis}]\clearpage
\printbibliography[heading=bibintoc, keyword={online}, title={Literaturverzeichnis}]\clearpage
%\printbibliography[heading=bibintoc, keyword={image}, title={Bildquellen}]\clearpage


\pagenumbering{arabic}
\fancyhead[L]{\nouppercase{\leftmark}} %Kopfzeile links
\input{chapter/Einleitung} \clearpage
\input{chapter/Informationsbeschaffung} \clearpage
\input{chapter/Konzeption} \clearpage
\input{chapter/Realisierung} \clearpage
\input{chapter/Tests} \clearpage
\input{chapter/Schlusswort} \clearpage
%\input{chapter/Beispiele} \clearpage
\input{chapter/Software} \clearpage

\pagenumbering{Roman}

%\printglossary[title={Glossar}] \clearpage
%\printglossary[style=dottedlocations,type=\acronymtype,title={Abkürzungsverzeichnis}] \clearpage
% Anhang

\appendix
%\fancyhead[L]{\nouppercase{\leftmark}} %Kopfzeile links
%\fancyhead[L]{\nouppercase{Anhang}} %Kopfzeile links
\input{chapter/Anhang}

\end{document}